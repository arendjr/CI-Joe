    ...
    attachTo: function(app) {
        ...
        function handleError(req, res, error) {
            ...
            if (!error.errorCode || !error.errorMessage) {
                error = Errors.serverError(error.message ? error.message : error);
            }

            res.status(error.httpStatus);
            delete error.httpStatus;

            var result = { errorCode: error.errorCode, errorMessage: error.errorMessage };
            if (error.message) {
        ...
        app.get("/api/workspaces/", function(req, res) {
            res.send({
                ...
                numItems: self.commandPost.workspaces.length
            });
        });
        ...
        app.post("/api/campaigns/:id/stop", function(req, res) {
            try {
                var campaignId = req.params.id;
                self.commandPost.stopCampaign(campaignId);
                res.send({});
            } catch(exception) {
                handleError(req, res, exception);
            }
        ...
        app.post("/api/missions/:id/start", function(req, res) {
            try {
                ...
                self.commandPost.startMission(missionId);
                res.send({});
            } catch(exception) {
                handleError(req, res, exception);
            }
        ...
        app.post("/api/missions/:id/stop", function(req, res) {
            try {
                var missionId = req.params.id;
                self.commandPost.stopMission(missionId);
                res.send({});
            } catch(exception) {
                handleError(req, res, exception);
            }
