...
_.extend(CommandPost.prototype, {
    ...
    removeCampaign: function(id) {
        ...
        if (result) {
            ...
            _.each(campaign.workspaces, function(workspace) {
    ...
    _bindMission: function(mission) {
        ...
        var self = this;
        mission.on("update:job-results", function(event) {
            var job = event.job;
            self.clients.notifyAll("missions:update-results", {
    ...
    _cleanupMission: function(missionId, options) {
        ...
        var isInUse = (
            _.any(this.campaigns.without(options.exceptCampaign), function(campaign) {
                return _.contains(_.flatten(_.pluck(campaign.phases, "missions")), missionId);
            })
        );
    ...
    _processCampaignMissions: function(campaignData) {
        ...
        _.each(campaignData.phases, function(phase) {
            phase.missions = _.map(phase.missions, function(mission) {
                if (typeof mission === "string") {
                    ...
                    return this.addMission(mission).id;
                }
            }, this);
        }, this);
        ...
        if (campaignData.id) {
            ...
            var originalMissions = _.flatten(_.pluck(campaign.phases, "missions"));
            var newMissions = _.flatten(_.pluck(campaignData.phases, "missions"));
            var removedMissions = _.diff(newMissions, originalMissions);
    ...
    _processCampaignWorkspaces: function(campaignData) {
        ...
        campaignData.workspaces = _.map(campaignData.workspaces, function(workspace) {
            if (typeof workspace === "string") {
                if (!this.workspaces.get(workspace)) {
                    throw Errors.notFound("Referenced workspace not found");
                }
                ...
                return this.addWorkspace(workspace).id;
            }
        }, this);
        ...
        if (campaignData.id) {
            ...
            _.each(removedWorkspaces, function(workspace) {
                this._cleanupWorkspace(workspace, { exceptCampaign: campaign });
            }, this);
        }
    },
        ...
        if (workspace) {
            if (typeof workspace === "string") {
                ...
                if (workspace.id) {
                    ...
                    workspace = this.addWorkspace(workspace).id;
                }
            }
        } else {
