 ...
 */
function CommandPost(config, barracks, slaveDriver, clientPool) {
...
_.extend(CommandPost.prototype, {
    ...
    addCampaign: function(campaignData) {
        ...
        this._setCampaignWorkspaces(campaign, campaignData.workspaces);
    ...
    updateCampaign: function(campaignData) {
        ...
        if (campaign) {
            ...
            campaign.setPhases(campaignData.phases);
    ...
    _updateCampaignWorkspaces: function(campaign, workspaces) {

        workspaces = _.map(workspaces, function(workspace) {
            if (typeof workspace === "string") {
                return workspace;
            } else {
                return this.barracks.addWorkspace(workspace);
            }
        }, this);

        var removedWorkspaces = _.diff(campaign.workspaces, workspaces);
        _.each(removedWorkspaces, function(workspace) {
            ...
            if (!isInUse) {
                this.barracks.removeWorkspace(workspace);
            }
        }, this);

        campaign.setWorkspaces(workspaces);
    }
