...
module.exports = Model.extend({
    ...
    initialize: function(config, options) {
         ...
         * Array of jobs.
         */
        this.set("jobs", _.map(this.config.jobs, function(jobConfig) {
            return new Job(jobConfig, { isMaster: options.isMaster, missionId: this.id });
        }, this));
         ...
         */
        this.set("standalone", config.standalone || true);
    ...
    addQueuedJob: function() {
        ...
        var self = this;
        job.on("change", function(event) {
            var key = event.key;
            if (key === "results") {
                self.fire("update:job-results", { job: job });
            } else if (key === "status") {
                if (event.value === "success") {
                    self.set("lastSuccess", job.results.endTime);
                } else if (event.value === "failed") {
                    self.set("lastFailure", job.results.endTime);
                }
                self.fire("update:job-status", { job: job });
            }
        });
